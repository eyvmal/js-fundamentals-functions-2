...
function timerStatus(timer) {
  if (timer <= 0) return "Phil's cake is ready!"
  else if (typeof timer === 'undefined') return "You didn't set a timer!"
...
function calculateQuantities(ingredients, layers) {
  const shoppingList = {}
  shoppingList.sugar = ingredients.includes('sugar') ? 100 * layers : 0
  shoppingList.eggs = ingredients.includes('eggs') ? 2 * layers : 0
  return shoppingList
}
